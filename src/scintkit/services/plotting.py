...
def individual_summary_plot(df, timecol="secbin", elmask=10):
    ...
    for i, prn in enumerate(prns):
        ...
        s4_mask1 = (
            (g["s4_quality_flag_1"] == 0) & (g["elev"] > elmask) & (g["elev"] < 91)
        )
        g.loc[~s4_mask1, "s4_1"] = np.nan

        s4_mask2 = (
            (g["s4_quality_flag_2"] == 0) & (g["elev"] > elmask) & (g["elev"] < 91)
        )
        ...
        ax2.set_ylabel(r"$S4 / \sigma_{\phi}$")
        ax2.set_xlabel("UT")
        ax2.set_ylim(0, 1)
    ...
    fig.suptitle(f"ScintPi Data - {day}", fontsize=12, y=0.95)
