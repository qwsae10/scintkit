...
def highpass_snr(df,in_col="snr1",out_col="detrended_snr1",tr=1,fs=None):
...
def highpass_phase(
    ...
    for prn, pos in df.groupby("prn", sort=False).indices.items():#make sure PRN is unique for each satellite
        phase = phase_all[pos]
...
def make_edge_gap_mask(time, phase, fs, gap_seconds=1, pad_seconds=180):
    ...
    return mask
def highpass_all_snrs(df,fs=None,tr=1):
    for i in range(1,4):
        if f"snr{i}" not in df.columns:
            continue
        in_col = f"snr{i}"
        out_col = f"detrended_snr{i}"
        frequency_col = f"freq_{i}"
        df = highpass_snr(df,in_col=in_col, out_col=out_col, fs = fs)
    return df
